...
def list_threads(db: Session = Depends(get_db)):
    return get_threads(db)


@router.post("/threads", response_model=InboxThreadRead)
def create_new_thread(thread_in: InboxThreadCreate, db: Session = Depends(get_db)):
    return create_thread(db, thread_in)
...
def update_existing_thread(thread_id: UUID, thread_in: InboxThreadUpdate, db: Session = Depends(get_db)):
    thread = get_thread(db, thread_id)
    if not thread:
        raise HTTPException(status_code=404, detail="Inbox thread not found")
...
def delete_existing_thread(thread_id: UUID, db: Session = Depends(get_db)):
    thread = get_thread(db, thread_id)
    if not thread:
        raise HTTPException(status_code=404, detail="Inbox thread not found")
...
def add_existing_item_to_thread(thread_id: UUID, item_id: UUID, db: Session = Depends(get_db)):
    thread = get_thread(db, thread_id)
    item = get_inbox_item(db, item_id)
...
def remove_existing_item_from_thread(thread_id: UUID, item_id: UUID, db: Session = Depends(get_db)):
    thread = get_thread(db, thread_id)
    item = get_inbox_item(db, item_id)
...
def analyze_existing_thread(thread_id: UUID, db: Session = Depends(get_db)):
    thread = get_thread(db, thread_id)
    if not thread:
        raise HTTPException(status_code=404, detail="Inbox thread not found")
    return analyze_thread(db, thread)


@router.post("/threads/{thread_id}/convert-to-task", response_model=TaskRead)
def convert_existing_thread_to_task(thread_id: UUID, db: Session = Depends(get_db)):
    thread = get_thread(db, thread_id)
    if not thread:
        raise HTTPException(status_code=404, detail="Inbox thread not found")
    return convert_thread_to_task(db, thread)
...
@router.post("/{inbox_item_id}/analyze", response_model=TaskExtractionResult)
def analyze_existing_item(inbox_item_id: UUID, db: Session = Depends(get_db)):
    item = get_inbox_item(db, inbox_item_id)
    if not item:
        raise HTTPException(status_code=404, detail="Inbox item not found")
    return analyze_item(db, item)
