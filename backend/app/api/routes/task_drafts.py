...
def list_task_drafts(db: Session = Depends(get_db)):
    return get_task_drafts(db)


@router.post("/", response_model=TaskDraftRead)
def create_new_task_draft(draft_in: TaskDraftCreate, db: Session = Depends(get_db)):
    return create_task_draft(db, draft_in)


@router.get("/{draft_id}", response_model=TaskDraftRead)
def read_task_draft(draft_id: UUID, db: Session = Depends(get_db)):
    draft = get_task_draft(db, draft_id)
    if not draft:
...
def update_existing_task_draft(draft_id: UUID, draft_in: TaskDraftUpdate, db: Session = Depends(get_db)):
    draft = get_task_draft(db, draft_id)
    if not draft:
        raise HTTPException(status_code=404, detail="Task draft not found")
...
def delete_existing_task_draft(draft_id: UUID, db: Session = Depends(get_db)):
    draft = get_task_draft(db, draft_id)
    if not draft:
        raise HTTPException(status_code=404, detail="Task draft not found")
...
def convert_existing_task_draft(draft_id: UUID, db: Session = Depends(get_db)):
    draft = get_task_draft(db, draft_id)
    if not draft:
        raise HTTPException(status_code=404, detail="Task draft not found")
